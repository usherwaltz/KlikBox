...
class YIT_Contact_Form {
    ...
    protected function _sendEmail()
    {
        ...
        if ( isset( $_POST['yit_action'] ) AND ( $_POST['yit_action'] == 'sendemail' OR $_POST['yit_action'] == 'sendemail-ajax' ) AND wp_verify_nonce( $_REQUEST['_wpnonce'], 'yit-sendmail' ) )
        {
            ...
            // set content type
            add_filter( 'wp_mail_content_type', array( &$this, 'yith_wp_mail_content_type' ) );
            add_filter( 'wp_mail_from',         array( &$this, 'yith_wp_mail_from' ) );
            add_filter( 'wp_mail_from_name',    array( &$this, 'yith_wp_mail_from_name' ) );
    ...
    function yith_wp_mail_content_type( $content_type ) {
        return 'text/html';
    }

    function yith_wp_mail_from( $from ) {
        return $from_email;
    }

    function yith_wp_mail_from_name( $from ) {
        return $from_name;
    }
