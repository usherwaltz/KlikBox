...
class YIT_Contact_Form {
    ...
    public function module( $name_or_id, $action, $echo = true )
    {
        ...
        if ( is_shop_installed() && is_product() ) {
            $sku = IS_WC_LESS_3_0 ? $GLOBALS['product']->sku : $GLOBALS['product']->get_sku();
            $product_id = IS_WC_LESS_3_0 ? $GLOBALS['product']->id : $GLOBALS['product']->get_id();
            $html .= "\t\t\t\t<input type=\"hidden\" name=\"yit_contact[sku]\" value=\"". $sku . "\" />";
            $html .= "\t\t\t\t<input type=\"hidden\" name=\"yit_contact[product_id]\" value=\"". $product_id . "\" />";
        }
